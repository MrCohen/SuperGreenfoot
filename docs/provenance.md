...
| `bluej/lib/images/greenfoot-icon-{32,48,256}.png`, `bluej/lib/images/greenfootvm.icns`, `bluej/package/greenfoot-installer-icon.png`, `bluej/package/winlaunch/greenfoot-icon-256-shadow.ico`, `greenfoot/resources/images/greenfoot.icns` and `greenfoot-icon-*` | Upstream icons replaced with the original Super Greenfoot logo (`docs/branding/logo/`, new; the full list is in `docs/branding/logo/APPLIED.md`). | release 0.1.1 |
| `installer/mac/{create-dmg.sh,set-dmg-layout.applescript}`, `installer/mac/assets/dmg-background.png` | DMG window shows the whole artwork (footer strip, taller window); icons centred in their cards; `.fseventsd` removed from the image and `.background` hidden and kept off the artwork. | release 0.1.1 |
| `version.properties`, `boot/src/main/java/bluej/Boot.java`, `README.md` | Version 0.1.1; run-from-source steps point at `v0.1.1`. | release 0.1.1 |
